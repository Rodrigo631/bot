...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            + 'VERSION:3.0\n' 
            + 'FN:+351 928 044 353\n' // full name
            + 'ORG:Owner MAGICA BOT;\n' // the organization of the contact
            + 'TEL;type=CELL;type=VOICE;waid=351928044353:+351 928 044 353 chama bb\n' // WhatsApp ID + phone number
            + 'END:VCARD'
blocked = ['351928044354@s.whatsapp.net']
...
async function starts() {
	...
	client.on('credentials-updated', () => {
		fs.writeFileSync('./馃惁.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'Login Info')
	})
	fs.existsSync('./馃惁.json') && client.loadAuthInfo('./馃惁.json')
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				num = anu.participants[0]
				teks = `Selamat Tinggal @${num.split('@')[0]}馃憢\nJangan Lupain Aku Ya!`
				client.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '鈱� AGUARDE UM POUCO IRM脙O 鈱�',
				success: '鉁旓笍 Bem sucedido 鉁旓笍',
				error: {
					stick: '[鉂梋 Falha, erro ocorreu ao converter imagem em adesivo 鉂�',
					Iv: '鉂� Link tidak valid 鉂�'
				},
				only: {
					group: '[鉂梋 Este comando só pode ser usado em grupos!  鉂�',
					ownerG: '[鉂梋 Perintah ini hanya bisa di gunakan oleh owner group! 鉂�',
					ownerB: '[鉂梋 Este comando só pode ser usado pelo dono do bot! 鉂�',
					admin: '[鉂梋 Este comando só pode ser usado por administradores de grupo! 鉂�',
					Badmin: '[鉂梋  鉂�',
				premi: '[鉂梋 ESTE COMANDO É APENAS UM USUÁRIO *PREMIUM*'
				}
			}
			const truth =[
        ...
        'drop emot "馃馃挩" setiap ngetik di gc/pc selama 1 hari.',
        'ucapin kata "Selamat datang di Who Wants To Be a Millionaire!" ke semua grup yang kamu punya',
        'marah虏 ga jelas ke penonton sw kamu urutan 30',
        'telfon mantan bilang kangen',
        'yanyiin reff lagu yang terakhir kamu setel',
        'vn mantan/crush/pacar kamu, bilang hi (namanya), mau telfon dong, bentar ajaa. aku kangen馃ズ馃憠馃徏馃憟馃徏"',
	       ...
	       const pushname = client.chats.get(mek.participant) === undefined ? (client.contacts[mek.key.remoteJid].notify ? client.contacts[mek.key.remoteJid].notify : "Kak") : (client.contacts[mek.participant].notify ? client.contacts[mek.participant].notify : "Kak")
			...
			case 'tagall':
				client.updatePresence(from, Presence.composing) 
					...
					mentions('鈺斺晲鈺愨湭銆� Mention All 銆欌湭鈺愨晲\n鈺犫灔'+teks+'鈺氣晲銆� CALIPH BOT 銆�', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '鈺斺晲鈺愨湭銆� Mention All 銆欌湭鈺愨晲\n鈺犫灔'+teks+'鈺氣晲銆� CALIPH BOT 銆�', text, {quoted: mek})
					break
                case 'tagall3':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '鈺斺晲鈺愨湭銆� Mention All 銆欌湭鈺愨晲\n鈺犫灔'+teks+'鈺氣晲銆� CALIPH BOT 銆�', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					teks += `  Total : ${groupMembers.length}\n`
					for (let mem of groupMembers) {
						teks += `鈺犫灔 ${mem.jid.split('@')[0]}@s.whatsapp.net\n`
						members_id.push(mem.jid)
					}
					reply('鈺斺晲鈺愨湭銆� Mention All 銆欌湭鈺愨晲\n鈺犫灔'+teks+'鈺氣晲銆� CALIPH BOT 銆�')
					break
				case 'clearall':
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*銆� INFO 銆�*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*銆� INFO 銆�*\n\n${body.slice(4)}`)
						}
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							client.sendMessage(_.jid, buff, image, {caption: `*銆� BC GROUP 銆�*\n*Group* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
							sendMess(_.jid, `*銆� BC GROUP 銆�*\n*Group* : ${groupName}\n\n${body.slice(6)}`)
						}
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'yahaha Lu kekick馃槀', text)
					}
					break
				case 'listadmins':
				case 'adminlist':
				...
				case 'toimg':
					...
					if (!isGroup) (!isQuotedSticker) return reply('鉂� reply stickernya um 鉂�')
					reply(mess.wait)
                	...
                	client.updatePresence(from, Presence.composing) 
					if (!isPremium) return reply (mess.only.premi)
					if (!isQuotedVideo) return reply('鉂� reply videonya um 鉂�')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('鉂� Gagal, pada saat mengkonversi video ke mp3 鉂�')
						buffer = fs.readFileSync(ran)
				...
				ez = `*鉃� Author :* ${data.author}\n*鉃� Quotes :* ${data.quotes}`
				reply(ez)
				break
				case '3dtext':
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
               ...
               client.updatePresence(from, Presence.composing) 
                ...
                if (data.error) return reply(data.error)
                hasil = `鈺犫灔 Tempat : ${data.respon.tempat}\n鈺犫灔 angin : ${data.respon.angin}\n鈺犫灔 cuaca : ${data.respon.cuaca}\n鈺犫灔 desk : ${data.respon.desk}\n鈺犫灔 kelembapan : ${data.respon.kelembapan}\n鈺犫灔 suhu : ${data.respon.suhu}\n鈺犫灔 udara : ${data.respon.udara}`
                reply(hasil)
                break
				case 'hidetag':
                client.updatePresence(from, Presence.composing) 
...
client.updatePresence(from, Presence.composing) 
			...
			break
               ...
               client.updatePresence(from, Presence.composing) 
                ...
                hasil = `鈺犫灔 internasional : ${data.international}\n鈺犫灔 nomor : ${data.nomor}\n鈺犫灔 operator : ${data.op}`
                reply(hasil)
                break
               case 'neonime':
					client.updatePresence(from, Presence.composing) 
               ...
               client.updatePresence(from, Presence.composing) 
                ...
                hasil = `鈺犫灔 shubuh : ${data.Subuh}\n鈺犫灔 Dzuhur : ${data.Dzuhur}\n鈺犫灔 Ashar : ${data.Ashar}\n鈺犫灔 Maghrib : ${data.Maghrib}\n鈺犫灔 isya : ${data.isya}`
                case 'semoji':
                ...
                case 'pinterest':
					...
					nimek =  n[Math.floor(Math.random() * n.length)];
					pok = await getBuffer(nimek)
					client.sendMessage(from, pok, image, { quoted: mek, caption: `饾悘饾悎饾悕饾悡饾悇饾悜饾悇饾悞饾悡\n\*Hasil Pencarian* : *${body.slice(11)}*`})
					break
                case 'block':
					client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
               ...
               case 'setppbot':
				...
				if (!isQuotedImage) return reply(`Kirim gambar dengan caption ${prefix}setbotpp atau tag gambar yang sudah dikirim`)
					...
					reply('Makasih profil barunya馃槜')
					break
                   case 'artinama':
                  client.updatePresence(from, Presence.composing) 
                   ...
                   case 'brainly':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'Sayonara馃憢', text) // ur cods
					}, 0)
                     break
                   case 'lirik':
                   client.updatePresence(from, Presence.composing) 
